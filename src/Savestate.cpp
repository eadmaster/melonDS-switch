...
Savestate::Savestate(const char* filename, bool save)
{
    ...
    /*
    // lazy init global quicksave_buf
    if(quicksave_buf == NULL)
    {
        quicksave_buf = calloc(QUICKSAVE_DEFAULT_SIZE, 1);  // init to 0
        if(!quicksave_buf)
        {
            printf("savestate in ram: cannot allocate memory\n", filename);
            Error = true;
            return;
        }
    }*/
